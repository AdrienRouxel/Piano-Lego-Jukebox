...
import { LPF2_SERVICE, LPF2_CHARACTERISTIC } from './lego/protocol.js';
 ...
 * CSS : on résout les teintes du thème une fois, et on vide le cache dès que
 * `data-theme` change.
 */
...
function themeColor(name, fallback) {
  if (!paletteCache.has(name)) {
    ...
    paletteCache.set(name, value || fallback);
  }
  return paletteCache.get(name);
}

new MutationObserver(() => paletteCache.clear()).observe(document.documentElement, {
  attributes: true,
...
class Trace {
  ...
  draw() {
    ...
    const color = themeColor(this.colorVar, this.fallback);
    if (this.fill) {
      ctx.lineTo(w, h);
      ctx.lineTo(0, h);
      ctx.closePath();
...
const CARDS = [
  {
    ...
    trace: { key: 'spectrum', kind: 'spectrum', height: 46 },
    fields: [
  ...
  },
  {
    ...
    trace: { key: 'notes', kind: 'trace', color: '--geek-notes', fallback: '#e0a94a', max: 12, autoScale: true, height: 34 },
    fields: [
  ...
  },
  {
    ...
    trace: { key: 'power', kind: 'trace', color: '--geek-power', fallback: '#74e0a6', max: 100, height: 34 },
    fields: [
      ['activity', 'Activité'],
      ['accent', 'Accent'],
      ['pwm', 'Consigne PWM'],
  ...
  },
  {
    ...
    trace: { key: 'ble', kind: 'trace', color: '--geek-ble', fallback: '#6fa8dc', max: 8, autoScale: true, height: 34 },
    fields: [
      ['gatt', 'Service GATT'],
      ['link', 'État du lien'],
      ['fw', 'Micrologiciel'],
  ...
  {
    title: 'Rendu · machine',
    trace: { key: 'fps', kind: 'trace', color: '--geek-fps', fallback: '#e07a5f', max: 70, height: 34 },
    fields: [
...
export class GeekMode {
   ...
   */
  constructor({ hub, getPlayer, getDriver, settings }) {
    this.hub = hub;
    ...
    this.hub.addEventListener('raw', (event) => this._onFrame(event.detail));
  }
  ...
  mount(parent = document.body) {
    ...
    for (const card of CARDS) {
      ...
      if (card.trace) {
        ...
        canvas.style.height = `${card.trace.height}px`;
        canvas.setAttribute('aria-hidden', 'true');
        article.append(canvas);
        this.traces.set(
          card.trace.key,
          card.trace.kind === 'spectrum'
            ? { spectrum: canvas.getContext('2d'), canvas }
            : new Trace(canvas, card.trace)
        );
      }
  ...
  setEnabled(enabled) {
    ...
    document.body.classList.toggle('geek-on', this.enabled);
    this._lastFrameAt = 0;
    if (!this.enabled) {
  ...
  _onFrame(detail) {
    ...
    if (this._pendingFrames.length > FRAME_LINES) this._pendingFrames.splice(0, this._pendingFrames.length - FRAME_LINES);
  ...
  update(now) {
    ...
    const frameMs = this._lastFrameAt ? now - this._lastFrameAt : 0;
    ...
    for (const trace of this.traces.values()) if (trace instanceof Trace) trace.draw();
  ...
  _drawSpectrum(player) {
    const entry = this.traces.get('spectrum');
    const { canvas, spectrum: ctx } = entry;
    const ratio = Math.min(2, window.devicePixelRatio || 1);
    ...
    ctx.fillStyle = themeColor('--geek-notes', '#e0a94a');
  ...
  _readMotion(driver) {
    ...
    this.set('sensor', this.hub.lastSensorValue === null ? 'aucune mesure' : `${this.hub.lastSensorValue} / 10`);
  ...
  _readBluetooth(now) {
    ...
    this.set('link', hub.connected ? `connecté · ${info.name ?? 'hub'}` : hub.status);
    ...
    this.set('battery', Number.isFinite(info.battery) ? `${info.battery} %${info.batteryType ? ` · ${info.batteryType.toLowerCase()}` : ''}` : '—');
    this.set(
      'supply',
      Number.isFinite(info.voltage)
        ? `${nf(info.voltage, 2)} V · ${nf(info.current ?? 0)} mA`
        : '—'
    );
  ...
  _readMachine(now) {
    ...
    this.set('viewport', `${window.innerWidth} × ${window.innerHeight} · DPR ${nf(window.devicePixelRatio, 2)}`);
  ...
  _flushFrames() {
    ...
    for (const frame of this._pendingFrames.reverse()) {
      ...
      const summary = document.createElement('i');
    ...
    while (this.stream.childElementCount > FRAME_LINES) this.stream.lastElementChild.remove();
